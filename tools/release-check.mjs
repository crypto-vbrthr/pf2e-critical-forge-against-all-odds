...
check(NARROW_ESCAPE_FORTITUDE_CARDS.length === 20, "Narrow Escapes Fortitude must contain twenty cards after the second pass.");
check(new Set(NARROW_ESCAPE_FORTITUDE_CARDS.map((card) => card.id)).size === 20, "Narrow Escapes Fortitude card IDs must be unique.");
...
check(narrowFortitudeFirst.length === 10, "Narrow Escapes Fortitude first pass must preserve ten content-batch-31 cards.");
check(narrowFortitudeFirst.filter((card) => card.effect).length === 9, "Narrow Escapes Fortitude first pass must preserve nine automated results.");
check(narrowFortitudeFirst.filter((card) => !card.effect).length === 1, "Narrow Escapes Fortitude first pass must preserve one manual result.");
check(narrowFortitudeSecond.length === 10, "Narrow Escapes Fortitude second pass must use ten content-batch-35 cards.");
check(narrowFortitudeSecond.filter((card) => card.effect).length === 8, "Narrow Escapes Fortitude second pass must contain eight automated results.");
check(narrowFortitudeSecond.filter((card) => !card.effect).length === 2, "Narrow Escapes Fortitude second pass must contain two manual results.");
...
check(NARROW_ESCAPE_REFLEX_CARDS.length === 20, "Narrow Escapes Reflex must contain twenty cards after the second pass.");
check(new Set(NARROW_ESCAPE_REFLEX_CARDS.map((card) => card.id)).size === 20, "Narrow Escapes Reflex card IDs must be unique.");
...
check(NARROW_ESCAPE_REFLEX_CARDS.filter((card) => card.metadata?.contentBatch === 36).length === 10, "Narrow Escapes Reflex second pass must contain ten batch-36 cards.");
