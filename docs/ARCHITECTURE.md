...
Bloodied Triumphs, Surrounded, Still Standing, and Giant-Slayer Moments are complete at 120 stable-ID cards each across their four specialized deck files. Narrow Escapes now contains a 20-card Attack deck plus 10-card Fortitude, Reflex, and Will decks in their dedicated card files, all wired through the shared factory and their matching specialized decks. The reviewed first 40 cards remain intact; Attack cards 11–20 use content batch 34 with eight automated and two manual escape results. The same-gate mechanical-superset regression guard now runs across all published Narrow Escape cards. Card filters and additive condition groups can bind specialized results to triggering traits, threat-count thresholds, current-opponent threat membership, level-gap escalation, participant size relations, or Narrow Escape danger-score escalation.
