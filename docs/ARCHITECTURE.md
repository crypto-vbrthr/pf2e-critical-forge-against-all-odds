...
Bloodied Triumphs and Surrounded, Still Standing are complete at 120 stable-ID cards each across their four specialized deck files. Giant-Slayer Moments currently contains 50 cards: twenty Attack cards after its second pass plus the reviewed first ten-card passes in Fortitude, Reflex, and Will. Card filters and additive condition groups can bind specialized results to triggering traits, threat-count thresholds, the current opponent being one of the counted melee threats, level-gap escalation, a verified larger-opponent relation, or an explicit signed size gap. Narrow Escapes remains the final empty foundation.
