...
Bloodied Triumphs, Surrounded, Still Standing, Giant-Slayer Moments, and Narrow Escapes are complete at 120 stable-ID cards each across their four specialized deck files. Narrow Escapes contains 30-card Attack, Fortitude, Reflex, and Will decks. Its first pass uses batches 30–33 with nine automated and one manual result per deck; its second pass uses batches 34–37 with eight automated and two manual results per deck; and its final pass uses batch 38 with eight automated and two manual results in every deck. The canonical exact-effect, strict same-gate mechanical-superset, and same-gate conceptual-footprint regression guards now run across the complete 480-card add-on. The release review also verifies 480 globally unique IDs, localized title uniqueness, English fallback synchronization, and German Remaster terminology. Card filters and additive condition groups can bind specialized results to triggering traits, threat-count thresholds, current-opponent threat membership, level-gap escalation, participant size relations, or Narrow Escape danger-score escalation.
