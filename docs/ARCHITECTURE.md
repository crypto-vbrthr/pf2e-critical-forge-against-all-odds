...
Bloodied Triumphs, Surrounded, Still Standing, and Giant-Slayer Moments are complete at 120 stable-ID cards each across their four specialized deck files. Narrow Escapes now contains first-pass Attack, Fortitude, Reflex, and Will decks at ten cards each in their dedicated card files, all wired through the shared factory and their matching specialized decks. The first 40-card review keeps all published mechanics and IDs intact and adds a same-gate mechanical-superset regression guard across the theme. Card filters and additive condition groups can bind specialized results to triggering traits, threat-count thresholds, current-opponent threat membership, level-gap escalation, participant size relations, or Narrow Escape danger-score escalation.
