...
Bloodied Triumphs and Surrounded, Still Standing are complete at 120 stable-ID cards each across their four specialized deck files. Giant-Slayer Moments currently contains 60 cards: twenty Attack and twenty Fortitude cards after their second passes plus the reviewed first ten-card passes in Reflex and Will. Card filters and additive condition groups can bind specialized results to triggering traits, threat-count thresholds, the current opponent being one of the counted melee threats, level-gap escalation, a verified larger-opponent relation, or an explicit signed size gap. Narrow Escapes remains the final empty foundation.
