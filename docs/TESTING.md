...
- nine automated and one manual result in each first-pass Narrow Escape deck; Attack, Fortitude, and Reflex second passes each add eight automated and two manual results;
- danger-score 4 and 5 escalation conditions in all four first-pass Narrow Escape decks, with a second score-4/score-5 pair now present in Attack, Fortitude, and Reflex;
...
Current `0.1.0-dev.37` baseline: **256/256 tests**, **99.53% line coverage**, **95.28% branch coverage**, and **98.96% function coverage**. The current build contains **440 published cards and 375 automated card effects**; Narrow Escapes contributes **80 cards, 68 automated effects, and 12 manual results** across all four specialized decks. The reviewed first 40 cards remain intact and every Narrow Escape deck is now at 20/30. Every automated Narrow Escape save effect remains on the saving actor, and the complete Narrow Escape exact-signature plus same-gate audits remain active. The second Will pass adds focused fear/mental gates, one additional score-4 and score-5 escalation, and distinct Step plus Recall Knowledge manual routes without resistance or immunity filler. Package-local release checks report **1304 localization keys per language**. The package-local release and schema checks cover all add-on contracts available in this workspace; the direct integration smoke test against the actual Critical Forge `1.0.0-rc` pack validator should be repeated in the integrated Critical Forge/Foundry workspace before tagging.
