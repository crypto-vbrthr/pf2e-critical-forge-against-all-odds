...
Current `0.1.0-dev.36` baseline: **253/253 tests**, **99.52% line coverage**, **95.23% branch coverage**, and **98.95% function coverage**. The current build contains **430 published cards and 367 automated card effects**; Narrow Escapes contributes **70 cards, 60 automated effects, and 10 manual results** across all four specialized decks. The reviewed first 40 cards remain intact, Attack, Fortitude, and Reflex are now at 20/30, every automated Narrow Escape save effect remains on the saving actor, and the complete Narrow Escape exact-signature plus same-gate audits remain active. The second Reflex pass adds distinct Leap/Step and Tumble Through/Stride manual escape routes without resistance or immunity filler. The package-local release and schema checks cover all add-on contracts available in this workspace; the direct integration smoke test against the actual Critical Forge `1.0.0-rc` pack validator should be repeated in the integrated Critical Forge/Foundry workspace before tagging.
