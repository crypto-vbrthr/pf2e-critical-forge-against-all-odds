...
- twenty Narrow Escape Reflex cards across content batches `32` and `36`, isolated to critical Reflex successes;
- twenty Narrow Escape Will cards across content batches `33` and `37`, isolated to critical Will successes;
- the dynamic Narrow Escape gate on every published Narrow Escape card;
- nine automated and one manual result in each first-pass Narrow Escape deck; all four second passes each add eight automated and two manual results;
- danger-score 4 and 5 escalation conditions in all four first-pass Narrow Escape decks, with a second score-4/score-5 pair present in Attack, Fortitude, Reflex, and Will;
...
Current `0.1.0-dev.37.1` review baseline: **259/259 tests**, **99.53% line coverage**, **95.37% branch coverage**, and **98.90% function coverage**. The current build contains **440 published cards and 375 automated card effects**; Narrow Escapes contributes **80 cards, 68 automated effects, and 12 manual results** across all four specialized decks. Every Narrow Escape deck remains at 20/30 with exactly two score-4 and two score-5 escalation cards. Every automated Narrow Escape save effect remains on the saving actor, the strict same-gate audit remains clean, and the new canonical signature audit normalizes component order, selector-array order, predicate order, and object-key order before comparing mechanics. The review separated four previously order-hidden duplicates while preserving every card ID and trigger gate. Package-local release checks report **1304 localization keys per language**. The package-local release and schema checks cover all add-on contracts available in this workspace; the direct integration smoke test against the actual Critical Forge `1.0.0-rc` pack validator should be repeated in the integrated Critical Forge/Foundry workspace before tagging.
