...
Package-local tests exercise the add-on's public Critical Forge extension contracts, provider schemas, pack lifecycle, rollback behavior, release metadata, and the Effect Engine component discriminator set across all 480 published cards. The `0.1.0-dev.38.3` hotfix expanded that discriminator guard from Bloodied-only coverage after Foundry correctly rejected an invalid `temporary-hp` component in the Surrounded pack. A Foundry smoke test remains necessary because hook timing, world settings, protected editor rendering, live diagnostics, and the host module's actual `1.0.0-rc` validator cannot be reproduced completely by the standalone Node workspace.

Current `0.1.0-dev.38.3` hotfix baseline: **271/271 tests**, **99.57% line coverage**, **95.51% branch coverage**, and **98.94% function coverage**. The build contains the complete **480 published cards**, with **407 automated card effects and 73 manual results**. The release-level canonical signature audit now covers all four themes, normalizing component order, selector-array order, predicate order, and object-key order before comparison; all **407 automated effects are canonically unique**. Strict same-gate mechanical-superset and conceptual selector-footprint audits are also clean across Bloodied Triumphs, Surrounded, Still Standing, Giant-Slayer Moments, and Narrow Escapes. Release guards additionally enforce stable/unique card IDs, unique localized titles in German and English, English fallback/localization synchronization, and global German Remaster terminology for bonus types, Auf dem Falschen Fuß, and Fortstoßen. Package-local release checks report **1416 localization keys per language**. The remaining pre-tag integration item is a direct validation against the actual Critical Forge `1.0.0-rc` host validator in the integrated Critical Forge/Foundry workspace.
